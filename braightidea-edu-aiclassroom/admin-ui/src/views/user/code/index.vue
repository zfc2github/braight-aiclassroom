<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="codeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="手机号" align="center" prop="number" />
...
<script>
import { listCode, getCode, delCode, addCode, updateCode } from "@/api/user/code";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除code发送信息编号为"' + ids + '"的数据项？').then(function() {
        return delCode(ids);
      }).then(() => {
        this.getList();
